		...
		public Vector getusedate(String userId){
			...
			try {
				...
				while(rs.next()){
					...
					sql ="select date_add(od.openDate,interval cl.useDate month),openDate FROM open_date od join cos_list cl on od.openCosNum = cl.cosNum where od.openUserId=?";
					...
					udto.setCalDate(rs.getDate("date_add(od.openDate,interval cl.useDate month)"));
